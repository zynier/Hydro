...
CUDA 提交使用 `nvcc -O3 --use_fast_math --extra-device-vectorization` 编译。快速数学运算可能采用精度较低但更快的设备指令，最终结果仍须满足本题 checker 的误差要求。TileLang 提交使用固定的 `tilelang==0.1.13` 环境，预热后的每次 `run_kernel` 调用与 CUDA 提交采用相同的 CUDA Event 计时和正确性检查。
