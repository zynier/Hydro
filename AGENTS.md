...
- PM2 状态：`/home/dongwu.chen/.local/state/hydro-conda/pm2`
- Hydro 文件存储：`/home/dongwu.chen/.local/share/hydro-conda/file`
- MongoDB 数据：`/home/dongwu.chen/.local/share/hydro-conda/mongodb`
- 用户级 systemd 单元：`hydro-conda.service`
- 当前版本：HydroOJ `5.0.4`、Node.js `24.13.0`、MongoDB `7.0.34`、Go Judge `v1.9.4`

当前由 PM2 管理三个进程：

| 进程 | 监听地址 | 用途 |
| --- | --- | --- |
| `hydro-conda` | `127.0.0.1:8888` | HydroOJ Web/API 服务 |
...
export PM2_HOME=/home/dongwu.chen/.local/state/hydro-conda/pm2
...
没有 sudo 时不能启用系统级 PM2 startup，也不能保证用户退出登录后服务继续运行。当前单元已启用到用户 session；机器重启后重新登录即可恢复。启用 linger 需要管理员权限：`loginctl enable-linger dongwu.chen`。

## 健康检查

```bash
pm2 ls
ss -ltnp | rg ':(27017|5050|8888)\b'
...
主要日志文件位于 `/home/dongwu.chen/.local/state/hydro-conda/`：
...
启动包装脚本位于 `/home/dongwu.chen/.local/bin/hydro-sandbox-start`，启动前会清理残留 scope；systemd 单元停止时也会清理。手工恢复：
...
```bash
df -h /home/dongwu.chen
df -ih /home/dongwu.chen
du -sh /home/dongwu.chen/.local/state/hydro-conda/*
```
...
```bash
backup_dir="/home/dongwu.chen/.local/backups/hydro-$(date +%F-%H%M%S)"
mkdir -p "$backup_dir"
systemctl --user stop hydro-conda.service || true
pm2 ls  # 确认 hydro-conda、hydro-sandbox、mongo-conda 均已停止
tar -C /home/dongwu.chen/.local/share/hydro-conda -czf "$backup_dir/data.tar.gz" mongodb file
